...
exports.create = async function (req, res) {
  ...
  if (data.password.toString().localeCompare(data.confirm_password.toString()) !== 0)
  ...
  let accType = data.type ? data.type.toString().toLowerCase() : undefined;
  if (!accType)
    throw { message: 'Please provide account type' };

  // create the account
  const accountData = await account.create(accType === 'p' ? 'owner' : 'admin');
  ...
  await user.account.add(userData.id, accountData.id, accType === 'p' ? 'owner' : 'admin');
